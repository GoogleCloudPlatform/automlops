...
def test_create_dockerfile(tmpdir):
    
    CloudRunBuilder.create_dockerfile(tmpdir)
    
    f = open(f'{tmpdir}/Dockerfile', 'r')
...
def test_create_requirements(tmpdir):
    
    CloudRunBuilder.create_requirements(tmpdir)
    
    f = open(f'{tmpdir}/requirements.txt', 'r')
...
def test_create_main(tmpdir):
    
    CloudRunBuilder.create_main(tmpdir)
    
    left_bracket = '{'
    right_bracket = '}'
    expected = (BuilderUtils.LICENSE +
    ...
    actual = f = open(f'{tmpdir}/main.py', 'r')
    
    assert expected == actual.read()