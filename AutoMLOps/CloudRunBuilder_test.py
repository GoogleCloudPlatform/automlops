...
DEFAULTS = {
    'gcp' : {
        'cloud_run_location': 'test_location',
        'cloud_run_name': 'test_cloudrun',
        'cloud_tasks_queue_location': 'test_location',
        'cloud_tasks_queue_name': 'test_queue',
        'pipeline_runner_service_account': 'test_service_acct',
        'project_id': 'test_project',
        'cloud_schedule_location': 'test_location',
        'cloud_schedule_name': 'test_schedule',
        'cloud_schedule_pattern': 'test_pattern'
    }
}

def test_formalize(tmpdir, mocker):
    # Ensure all files are created
    mocker.patch('AutoMLOps.BuilderUtils.read_yaml_file',
                 return_value = DEFAULTS)
    mocker.patch('AutoMLOps.BuilderUtils.execute_process',
                return_value = '') 
    CloudRunBuilder.formalize(tmpdir, tmpdir)
    assert os.listdir(tmpdir + 'cloud_run') == ['queueing_svc', 'run_pipeline']
    assert set(os.listdir(tmpdir + 'cloud_run/run_pipeline')) == set(['main.py', 'requirements.txt', 'Dockerfile'])
    assert set(os.listdir(tmpdir + 'cloud_run/queueing_svc')) == set(['main.py', 'requirements.txt'])

def test_create_dockerfile():
    # Ensure correct string is created
    assert CloudRunBuilder._create_dockerfile() == (
...
def test_create_cloudrun_reqs():
    # Ensure correct string is created
    assert CloudRunBuilder._create_cloudrun_reqs() == (
        'kfp\n'
...
def test_create_queueing_svc_reqs():
    # Ensure correct string is created
    assert CloudRunBuilder._create_queueing_svc_reqs() == (
        'google-cloud\n'
        'google-cloud-tasks\n'
...
def test_create_cloud_run_code():
    # Ensure correct string is created
    left_bracket = '{'
    right_bracket = '}'
    assert CloudRunBuilder._create_cloud_run_code() == (BuilderUtils.LICENSE +
...
def test_create_queueing_svc_code():
    # Ensure correct string is created
    left_bracket = '{'
    right_bracket = '}'
    assert CloudRunBuilder._create_queueing_svc_code(DEFAULTS) == (BuilderUtils.LICENSE +
