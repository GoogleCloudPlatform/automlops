...
def _create_pulumi_yaml_jinja(
    ...
    with open(PULUMI_TEMPLATE_PATH / 'Pulumi.yaml.jinja', 'r', encoding='utf-8') as f:
...
def _create_pulumi_dev_yaml_jinja(
    ...
    with open(PULUMI_TEMPLATE_PATH / 'Pulumi.dev.yaml.jinja', 'r', encoding='utf-8') as f:
        template = Template(f.read())
        return template.render(
            generated_license=GENERATED_LICENSE,
            project_id=project_id,
...
def _create_main_python_jinja(
    ...
    with open(PULUMI_TEMPLATE_PATH / 'python/__main__.py.jinja', 'r', encoding='utf-8') as f:
        template = Template(f.read())
        return template.render(
            generated_license=GENERATED_LICENSE,
            artifact_repo_name=artifact_repo_name,
