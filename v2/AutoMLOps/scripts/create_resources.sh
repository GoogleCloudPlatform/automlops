...
if ! (gcloud scheduler jobs list --project="$PROJECT_ID" --location="$CLOUD_SCHEDULE_LOCATION" | grep --fixed-strings "AutoMLOps-schedule"); then
  ...
  echo "Cloud Scheduler AutoMLOps resource already exists in project $PROJECT_ID"
